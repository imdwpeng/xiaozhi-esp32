...
bool AudioCodec::SetOutputSampleRate(int sample_rate) {
    ...
    if (enable_ret != ESP_OK) {
        ...
        ESP_LOGI(TAG, "Successfully enabled I2S TX channel with new sample rate");
    }
    
    if (ret == ESP_OK) {
        output_sample_rate_ = sample_rate;
        ESP_LOGI(TAG, "Successfully changed output sample rate to %d Hz", output_sample_rate_);
        return true;
    } else {
        ESP_LOGE(TAG, "Failed to change output sample rate: %s", esp_err_to_name(ret));
        
        // 如果重新配置失败，尝试恢复到原始采样率
        if (original_output_sample_rate_ > 0 && sample_rate != original_output_sample_rate_) {
            ESP_LOGI(TAG, "Attempting to restore original sample rate: %d Hz", original_output_sample_rate_);
            // 简化恢复流程，只更新变量，不再次尝试硬件重配置
            output_sample_rate_ = original_output_sample_rate_;
        }
        return false;
    }
}
